...
3. Execute the Kronos project:
   ...
   **Note: There are slight differences in the script parameters between this quick start and the one for Kronos with Speeding-Dumbo. The last two parameters (representing each node's transaction queue size and the total number of cross-shard transactions, respectively) differ because each node proposes B/N transactions per round when using Speeding-Dumbo, whereas it proposes B transactions per round when using Rotating-Hotstuff. B (the 4th parameter in the quick start command) refers to the BFT protocol batch size, and N (the 2nd parameter) represents the number of nodes in each shard.*
